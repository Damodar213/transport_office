...
export async function GET() {
  try {
    ...
    const todaySummary = {
      newRegistrations: weeklyUsers,
      ordersProcessed: ordersToday,
      documentsVerified: pendingReviews,
      issuesResolved: Math.floor(Math.random() * 10) + 1 // Mock for now
    }

    // --- Dynamic Pending Actions ---
    const pendingActions = {
      documentReviews: pendingReviews,
      orderAssignments: Math.floor(Math.random() * 10) + 1, // Mock for now
      userVerifications: Math.floor(Math.random() * 5) + 1, // Mock for now
      supportTickets: Math.floor(Math.random() * 5) + 1 // Mock for now
    }
    ...
    if (pendingReviews > 5) {
      systemAlerts.push({
        type: "warning",
        message: "High pending review queue",
        icon: "AlertTriangle"
      })
    }
    ...
    if (weeklyUsers === 0) {
      systemAlerts.push({
        type: "info",
        message: "No new registrations this week",
        icon: "Clock"
      })
    } else {
      systemAlerts.push({
