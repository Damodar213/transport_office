...
import { getAllStates, getDistrictsByState, District as StateDistrict } from "@/lib/indian-states-districts"
...
export function TransportRequests({ onDataChange }: TransportRequestsProps) {
  ...
  const [fromDistrictsFiltered, setFromDistrictsFiltered] = useState<StateDistrict[]>([])
  const [toDistrictsFiltered, setToDistrictsFiltered] = useState<StateDistrict[]>([])

  // Handle state selection and update districts
  const handleFromStateChange = (stateName: string) => {
    const districts = getDistrictsByState(stateName)
    setFromDistrictsFiltered(districts)
  }

  const handleToStateChange = (stateName: string) => {
    const districts = getDistrictsByState(stateName)
    setToDistrictsFiltered(districts)
  }
  ...
  // Load districts when editing a request
  useEffect(() => {
    if (editingRequest?.from_state) {
      const fromDistricts = getDistrictsByState(editingRequest.from_state)
      setFromDistrictsFiltered(fromDistricts)
    }
    if (editingRequest?.to_state) {
      const toDistricts = getDistrictsByState(editingRequest.to_state)
      setToDistrictsFiltered(toDistricts)
    }
  }, [editingRequest])
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select 
                      name="fromState" 
                      defaultValue={editingRequest?.from_state || ""} 
                  ...
                  <div className="space-y-2">
                    ...
                    <Select name="fromDistrict" defaultValue={editingRequest?.from_district || ""} required>
                      ...
                      <SelectContent>
                        {fromDistrictsFiltered.length === 0 ? (
                          <SelectItem value="loading" disabled>
                            {editingRequest?.from_state ? "No districts available" : "Select state first"}
                          </SelectItem>
                        ) : (
                          fromDistrictsFiltered.map((district) => (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select 
                      name="toState" 
                      defaultValue={editingRequest?.to_state || ""} 
                      onValueChange={handleToStateChange}
                      required
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select state" />
                  ...
                  <div className="space-y-2">
                    ...
                    <Select name="toDistrict" defaultValue={editingRequest?.to_district || ""} required>
                      ...
                      <SelectContent>
                        {toDistrictsFiltered.length === 0 ? (
                          <SelectItem value="loading" disabled>
                            {editingRequest?.to_state ? "No districts available" : "Select state first"}
                          </SelectItem>
                        ) : (
                          toDistrictsFiltered.map((district) => (
                            <SelectItem key={district.id} value={district.name}>
                              {district.name}
                            </SelectItem>
                          ))
