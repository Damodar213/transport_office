...
import { capitalizeName, capitalizePlace } from "@/lib/name-utils"
import { getAllStates, getDistrictsByState } from "@/lib/indian-states-districts"
...
export function TransportOrders({ onDataChange }: SupplierVehicleLocationProps) {
  ...
  // Fetch districts from admin API with fallback and retry
  const fetchDistricts = async (retryCount = 0) => {
    try {
      ...
      if (response.ok) {
        ...
        if (districts.length > 0) {
          ...
          setDistrictsRetryCount(0) // Reset retry count on success
        } else {
          // Database is empty, use fallback data
          console.log("Database districts empty, using fallback data")
          useFallbackData()
        }
      } else {
        console.error("Failed to fetch districts from API")
        if (retryCount < 2) {
          // Retry up to 2 times with exponential backoff
          const delay = Math.pow(2, retryCount) * 1000 // 1s, 2s, 4s
          console.log(`Retrying districts fetch in ${delay}ms (attempt ${retryCount + 1})`)
          setTimeout(() => {
            setDistrictsRetryCount(retryCount + 1)
            fetchDistricts(retryCount + 1)
          }, delay)
          return
        } else {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {states.length === 0 && !isLoadingDistricts && (
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={
                        isLoadingDistricts 
                          ? `Loading states...${districtsRetryCount > 0 ? ` (Retry ${districtsRetryCount})` : ''}` 
                          : states.length === 0 
                            ? "No states available" 
                            : "Select state"
                      } />
                    ...
                    <SelectContent>
                      {states.length > 0 ? (
                        ...
                        <div className="px-2 py-1.5 text-sm text-muted-foreground">
                          {isLoadingDistricts ? "Loading..." : "No states available"}
                        </div>
                      )}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {(selectedState || editingOrder?.state) && districtsByState[selectedState || editingOrder?.state || '']?.map((district: string) => (
                ...
                {orders.map((order, index) => (
                  <TableRow key={`${order.id}-${order.vehicle_number}-${index}`}>
                    <TableCell>
                      ...
                      <div className="text-sm text-muted-foreground">{capitalizePlace(order.state)}</div>
                    ...
                    <TableCell>
                      {order.driver_name ? (
                        <div className="font-medium text-blue-600">{capitalizeName(order.driver_name)}</div>
                      ) : (
