...
export function RecentOrders() {
  ...
  const fetchRecentOrders = async () => {
    try {
      ...
      const userCheckResponse = await fetch("/api/check-current-user")
      const userCheck = await userCheckResponse.json()
      
      console.log("Current user check:", userCheck)
      
      if (!userCheck.success || userCheck.user.role !== 'supplier') {
        console.error("User is not a supplier, cannot fetch orders")
        setError(`Access denied: Current user role is ${userCheck.user?.role || 'unknown'}. Only suppliers can access this section.`)
        return
      }
      
      console.log("User is authenticated as supplier, fetching recent orders...")
