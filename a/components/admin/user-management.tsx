...
import { capitalizeName, capitalizeWords } from "@/lib/name-utils"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function UserManagement() {
  ...
  const applyFilters = () => {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter((user) => {
        ...
        const gst = user.gstNumber?.toLowerCase() || ""
        return name.includes(q) || company.includes(q) || uid.includes(q) || gst.includes(q)
      })
    }
  ...
  const handleExportExcel = () => {
    const exportData: ExportableUser[] = filteredUsers.map(user => ({
      id: user.id,
      userId: user.userId,
      name: user.role === 'buyer' ? user.companyName : user.name,
      email: user.role === 'buyer' ? 'N/A' : user.email,
      mobile: user.role === 'buyer' ? 'N/A' : user.mobile,
      role: user.role,
            ...
            <TableBody>
              {filteredUsers.map((user) => (
                <TableRow key={user.id}>
                  <TableCell>
                    <div>
                      <div className="font-medium">{user.role === 'buyer' ? capitalizeWords(user.companyName) : capitalizeName(user.name)}</div>
                      <div className="text-sm text-muted-foreground">{user.userId}</div>
                      {user.role !== 'buyer' && user.email && (
                  ...
                  <TableCell>
                    <div>
                      <div className="font-medium">{capitalizeWords(user.companyName)}</div>
                    ...
                    <div className="flex items-center gap-2">
                      <Dialog>
                        ...
                        <DialogContent className="max-w-2xl">
                          <DialogHeader>
                            <DialogTitle>User Details - {user.role === 'buyer' ? user.companyName : user.name}</DialogTitle>
                            ...
                            {user.role === 'buyer' ? (
                              <>
                                <div>
                                  ...
                                  <p className="text-sm text-muted-foreground">{user.companyName || "Not provided"}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-muted-foreground">{user.gstNumber || "Not provided"}</p>
                                </div>
                                <div>
                                  <label className="text-sm font-medium">Role</label>
          ...
          {deleteDialog.user && (
            <div className="py-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="space-y-2">
                  {deleteDialog.user.role === 'buyer' ? (
                    <>
                      <div><strong>Company:</strong> {deleteDialog.user.companyName || 'N/A'}</div>
                      <div><strong>User ID:</strong> {deleteDialog.user.userId}</div>
                      <div><strong>GST:</strong> {deleteDialog.user.gstNumber || 'N/A'}</div>
