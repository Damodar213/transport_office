...
import { getAllStates, getDistrictsByState, District } from "@/lib/indian-states-districts"
...
export function OrderAssignment() {
  ...
  const [fromDistrictsFiltered, setFromDistrictsFiltered] = useState<District[]>([])
  const [toDistrictsFiltered, setToDistrictsFiltered] = useState<District[]>([])
  ...
  const handleFromStateChange = (stateName: string) => {
    setManualOrder((prev) => ({ ...prev, fromState: stateName, fromDistrict: "" }))
  ...
  const handleToStateChange = (stateName: string) => {
    setManualOrder((prev) => ({ ...prev, toState: stateName, toDistrict: "" }))
    const districts = getDistrictsByState(stateName)
                      ...
                      if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
                        ...
                        navigator.clipboard.writeText(whatsappUrl).then(() => {
                          toast({
                            ...
                            description: `WhatsApp popup was blocked. WhatsApp URL copied to clipboard for ${order.companyName || order.contactPerson}. Paste it in a new tab.`,
                            variant: "destructive",
                            duration: 8000
                          })
                        }).catch(() => {
                        ...
                        <SelectContent>
                          {fromDistrictsFiltered.length === 0 ? (
                            <SelectItem value="loading" disabled>
                              {manualOrder.fromState ? "No districts available" : "Select state first"}
                            </SelectItem>
                          ) : (
                      ...
                      >
                        ...
                        <SelectContent>
                          {toDistrictsFiltered.length === 0 ? (
                            <SelectItem value="loading" disabled>
                              {manualOrder.toState ? "No districts available" : "Select state first"}
                            </SelectItem>
                          ) : (
