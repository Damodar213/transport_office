    ...
    try {
      ...
      if (orderType === 'manual_order') {
        ...
        if (orderDetailsCheck.rows.length === 0) {
          ...
          console.log("Manual order submission details:", orderDetailsCheck.rows[0])
          
          // Check if manual_orders table has the required columns
          const manualOrderCheck = await dbQuery(
            "SELECT * FROM manual_orders WHERE id = $1",
            [orderDetailsCheck.rows[0].order_id]
          )
          console.log("Manual order details:", manualOrderCheck.rows[0])
        ...
        console.log("Created accepted request:", acceptedRequestResult.rows[0])
        }
      } else {
