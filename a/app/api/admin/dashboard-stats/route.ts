...
export async function GET() {
  try {
    ...
    // Get pending reviews count (pending documents from all document tables)
    let pendingReviews = 0
    let supplierDocsCount = 0
    let driverDocsCount = 0
    let vehicleDocsCount = 0
    
    try {
      // Count pending supplier documents (PAN, GST)
      const supplierDocsResult = await dbQuery(`
        SELECT COUNT(*) as count 
        FROM supplier_documents 
        WHERE status = 'pending' 
        AND document_type IN ('pan', 'gst')
      `)
      supplierDocsCount = parseInt(supplierDocsResult.rows[0].count) || 0
      pendingReviews += supplierDocsCount
    } catch (error) {
      console.log("Supplier documents table not found or error:", error)
    }
    
    try {
      // Count pending driver documents (driving licenses)
      const driverDocsResult = await dbQuery(`
        SELECT COUNT(*) as count 
        FROM driver_documents 
        WHERE status = 'pending' 
        AND document_type = 'license'
      `)
      driverDocsCount = parseInt(driverDocsResult.rows[0].count) || 0
      pendingReviews += driverDocsCount
    } catch (error) {
      console.log("Driver documents table not found or error:", error)
    }
    
    try {
      // Count pending vehicle documents (RC cards)
      const vehicleDocsResult = await dbQuery(`
        SELECT COUNT(*) as count 
        FROM vehicle_documents 
        WHERE status = 'pending' 
        AND document_type = 'rc'
      `)
      vehicleDocsCount = parseInt(vehicleDocsResult.rows[0].count) || 0
      pendingReviews += vehicleDocsCount
    } catch (error) {
      console.log("Vehicle documents table not found or error:", error)
    }
    
    console.log(`Total pending document reviews: ${pendingReviews}`)
    console.log("Pending reviews breakdown:")
    console.log("- Supplier documents (PAN, GST):", supplierDocsCount)
    console.log("- Driver documents (licenses):", driverDocsCount) 
    console.log("- Vehicle documents (RC):", vehicleDocsCount)
    ...
    const currentPeriodOrdersResult = await dbQuery(
      `SELECT
    ...
    const previousPeriodOrdersResult = await dbQuery(
      `SELECT
         COUNT(*) FILTER (WHERE status = 'confirmed') as completed_count,
         COUNT(*) as total_count
       FROM suppliers_vehicle_location
    ...
    const recentUsersResult = await dbQuery(
      `SELECT u.id, u.user_id, u.role, u.created_at, u.name 
       FROM users u 
