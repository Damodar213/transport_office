...
export async function PATCH(request: NextRequest) {
  return withAuth(async (session) => {
    return withDatabase(async () => {
      ...
      const result = await dbQuery(`UPDATE driver_documents 
...
export async function DELETE(request: NextRequest) {
  return withAuth(async (session) => {
    return withDatabase(async () => {
      ...
      const getResult = await dbQuery(`SELECT document_url FROM driver_documents WHERE id = $1`,
        [id])
      )
      ...
      const result = await dbQuery(`DELETE FROM driver_documents WHERE id = $1 RETURNING *`,
        [id])
      )
