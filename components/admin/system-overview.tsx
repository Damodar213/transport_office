      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">₹{stats.totalRevenue.count}</div>
        ...
        {stats && (
          <>
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  <div className="text-2xl font-bold">{stats.systemUptime.value}%</div>
                  <div className="flex items-center gap-1 text-sm text-green-600">
                    {renderTrendIcon(stats.systemUptime.trend)}
                    {stats.systemUptime.change}
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-1">{stats.systemUptime.description}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  <div className="text-2xl font-bold">{stats.averageResponseTime.value}</div>
                  <div className="flex items-center gap-1 text-sm text-green-600">
                    {renderTrendIcon(stats.averageResponseTime.trend)}
                    {stats.averageResponseTime.change}
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-1">{stats.averageResponseTime.description}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  <div className="text-2xl font-bold">{stats.userSatisfaction.value}/5</div>
                  <div className="flex items-center gap-1 text-sm text-green-600">
                    {renderTrendIcon(stats.userSatisfaction.trend)}
                    +{stats.userSatisfaction.change}
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-1">{stats.userSatisfaction.description}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  <div className="text-2xl font-bold">{stats.orderSuccessRate.value}%</div>
                  <div className="flex items-center gap-1 text-sm text-green-600">
                    {renderTrendIcon(stats.orderSuccessRate.trend)}
                    {stats.orderSuccessRate.change}
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-1">{stats.orderSuccessRate.description}</p>
              </CardContent>
            </Card>
          </>
        )}
              ...
              <>
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span>{stats.systemHealth.databasePerformance.label}</span>
                    <span>{stats.systemHealth.databasePerformance.value}%</span>
                  </div>
                  <Progress value={stats.systemHealth.databasePerformance.value} className="h-2" />
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span>{stats.systemHealth.apiResponseTime.label}</span>
                    <span>{stats.systemHealth.apiResponseTime.value}%</span>
                  </div>
                  <Progress value={stats.systemHealth.apiResponseTime.value} className="h-2" />
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span>{stats.systemHealth.storageUsage.label}</span>
                    <span>{stats.systemHealth.storageUsage.value}%</span>
                  </div>
                  <Progress value={stats.systemHealth.storageUsage.value} className="h-2" />
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span>{stats.systemHealth.userActivity.label}</span>
                    <span>{Math.round(stats.systemHealth.userActivity.value)}%</span>
                  </div>
                  <Progress value={stats.systemHealth.userActivity.value} className="h-2" />
                </div>
              </>
            )}
          </CardContent>
